...
# data folders
data_dir="../data/Conv_noregul"
base_model=Conv
regularization=0.0  # regularization constant for the loss function

# generating initial dataset
python3 gen_dataset.py --dset_path "${data_dir}/original_dset-ubyte.gz"

# running autoencoders chain
python3 make_chain.py --dset_dir "$data_dir" --model_type "$base_model" --regul_const "$regularization"  >  "${data_dir}/report.out"
